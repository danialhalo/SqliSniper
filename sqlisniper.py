...
def validate_sql_injection(url, header, payload, discord_webhook=None, proxy=None):
    ...
    try:
        ...
        print(colored(f"{Fore.BLUE}[False-Positive Check]{Style.RESET_ALL} {Fore.WHITE}Setting sleep time to 15 seconds: Server Response Time is {response_15s.elapsed.total_seconds()} seconds{Style.RESET_ALL}", 'white'))
        ...
        if response_15s.elapsed.total_seconds() >= 14:
            ...
            print(colored(f"{Fore.BLUE}[False-Positive Check]{Style.RESET_ALL} {Fore.WHITE}Setting sleep time to 5 seconds: Server Response Time is {response_5s.elapsed.total_seconds()} seconds{Style.RESET_ALL}", 'white'))



            if response_5s.elapsed.total_seconds() < 7:
                ...
                print(colored(f"[False-Positive]  {url} is false positive. Test manually.", 'white'))
        else:
            print(colored(f"[False-Positive]  {url} is false positive. Test manually.", 'white'))
